...
class AutoencoderRNN_VAE_v2(nn.Module):
    ...
    def forward(self, emotion=None, _3dmm=None, **kwargs):
        ...
        # emotion.shape: (seq_len==50, batch_size==480, emotion_dim==25)

        # original code from DLow repository
        h_x = self._encode(emotion)  # h_x.shape: (batch_size, dim)
        mu = self.fc_mu_enc(h_x)  # mu.shape: (batch_size, dim)
        logvar = self.fc_logvar_enc(h_x)  # logvar.shape: (batch_size, dim)
        ...
        Y_r = self._decode(z)  # Y_r.shape: (seq_len==50, batch_size, emotion_dim==25)
        ...
        return {
            "prediction": Y_r,
            "target": target_emotion,  # we are autoencoding
